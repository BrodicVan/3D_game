...
namespace hw_6
{
    ...
    {
        void start(bool if_phy);
        void restart(bool if_phy);
        int get_game_status();
        int get_score();
        int get_bullct();
    ...
    public class UserGUI : MonoBehaviour
    {
        ...
        void Start()
        {
            cross = Resources.Load<Texture>("hw_6/Cross2");
        }
        ...
        void OnGUI()
        {
            ...
            if(game_status==0)
            {
                ...
                if(GUI.Button(new Rect(Screen.width/2-60-100,Screen.height/2+40,120,30),"运动模式",start_fontStyle))
                    controller.start(false);
                if(GUI.Button(new Rect(Screen.width/2-60+100,Screen.height/2+40,120,30),"物理模式",start_fontStyle))
            ...
            else if(game_status==2)
            {
                ...
                if(GUI.Button(new Rect(Screen.width/2-60-100,Screen.height/2+40,120,30),"运动模式",restart_fontStyle))
                    controller.restart(false);
                if(GUI.Button(new Rect(Screen.width/2-60+100,Screen.height/2+40,120,30),"物理模式",restart_fontStyle))
